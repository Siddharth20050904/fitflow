...
export async function getStoreProducts(adminId: string) {
  try {
    ...
    if (!store) {
      return { ok: false, products: [], message: "Store not found" }
    }

    return { ok: true, products: store.products, message: "Products fetched successfully" }
  } catch (error) {
...
export async function createProduct(input: CreateProductInput) {
  try {
    const { adminId, name, description, price, stock, category } = input
    ...
    if (!store) {
      store = await prisma.store.create({
        data: {
          adminId,
          name: "Gym Store",
          description: "Official gym supplement store",
        },
      })
    }
